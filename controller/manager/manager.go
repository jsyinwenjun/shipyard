...
func (m DefaultManager) SaveServiceReg(serviceReg *auth.ServiceReg) error {
	...
	if reg != nil {
		...
		if _, err := r.Table(tblNameServiceReg).Filter(map[string]string{"service_name": serviceReg.ServiceName}).Update(updates).RunWrite(m.session); err != nil {
			return err
		}
...
func (m DefaultManager) DeleteServiceReg(serviceReg *auth.ServiceReg) error {
	res, err := r.Table(tblNameServiceReg).Filter(map[string]string{"service_name": serviceReg.ServiceName}).Delete().Run(m.session)
	if err != nil {
		return err
	}
